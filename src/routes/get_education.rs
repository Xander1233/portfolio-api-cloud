...
pub async fn get_education(state: web::Data<DynamoDbAppState>) -> actix_web::Result<HttpResponse> {
    let id: i64 = 1;
    let section_type = "EDUCATION";
    ...
    match get_section::<serde_json::Value>(&state.ddb, &state.table, id, section_type, state.cache.clone(), state.cache_ttl).await {
        ...
        Ok(None) => {
            Ok(HttpResponse::NotFound().body("Education section not found"))
        }
        Err(err) => {
            tracing::error!(
                task = "Get Education Section",
                result = "failure",
                error = %err,
                "Failed to retrieve education section from DynamoDB"
            );
